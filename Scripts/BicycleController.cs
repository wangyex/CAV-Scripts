...
namespace SBPScripts
{
    ...
    public class BicycleController : MonoBehaviour
    {
        ...
        // public SteamVR_Input_Sources handType; // Set this to left or right hand

        // Calibration positions for turning movement based on Left hand controller Z positions
        // public float leftTurnPositionZ;
        // public float straightPositionZ;
        // public float rightTurnPositionZ;
        ...
        // Functions to record the positions, also requires the Editor script to work
        // public void CalibrateLeftTurn()
        // {
        //     leftTurnPositionZ = leftHandPose.GetLocalPosition(SteamVR_Input_Sources.LeftHand).z;
        // }

        // public void CalibrateStraight()
        // {
        //     straightPositionZ = leftHandPose.GetLocalPosition(SteamVR_Input_Sources.LeftHand).z;
        // }

        // public void CalibrateRightTurn()
        // {
        //     rightTurnPositionZ = leftHandPose.GetLocalPosition(SteamVR_Input_Sources.LeftHand).z;
        // }
        void Awake()
        {
            transform.rotation = Quaternion.Euler(0, transform.rotation.eulerAngles.y, 0);
            triggerPress = SteamVR_Actions.default_GrabPinch;
        }
        ...
        {
            //Breaking Speed
            float decelerationRate = 15f;
            //cache rb velocity
            float currentSpeed = rb.velocity.magnitude;
            //Lefthand controller accelerator speed
            float constantSpeed = 5f;
            turnAngle = BlyncTurnAngle.value - 30f;
            BlyncSpeed = BlyncSensorSpeed.value;
            Debug.Log($"Steering Angle: {turnAngle}, Sensor Speed: {BlyncSpeed}");


            Vector3 leftHandPosition = leftHandPose.GetLocalPosition(SteamVR_Input_Sources.LeftHand);
            Vector3 rightHandPosition = rightHandPose.GetLocalPosition(SteamVR_Input_Sources.RightHand);

            
            if (triggerPress.GetState(SteamVR_Input_Sources.RightHand))
                {
                    //Implementing braking using right controller trigger
                    currentSpeed -= decelerationRate * Time.deltaTime;
                    currentSpeed = Mathf.Max(0, currentSpeed); // Ensure speed doesn't go below 0
                    Vector3 decelVelocity = transform.forward * currentSpeed;
                    Debug.Log("Applying brakes");
                    rb.velocity = decelVelocity;
                }
            if (triggerPress.GetState(SteamVR_Input_Sources.LeftHand))
                {
                    // Vector3 constVelocity = transform.forward * constantSpeed;
                    // rb.velocity = constVelocity;
                    rWheelRb.AddTorque(transform.right * constantSpeed * 20f);
                    Debug.Log("constant accel");
                }
            float steeringValue = turnAngle * -0.02f;
            // Now printing both Z positions in one line
            //Debug.Log($"Left hand Z: {leftHandPosition.z}, Steering Value: {steeringValue}");
            float fixedSteering = 5f;
            //Physics based Steering Control.
            fPhysicsWheel.transform.rotation = Quaternion.Euler(transform.rotation.eulerAngles.x, transform.rotation.eulerAngles.y + steeringValue * steerAngle.Evaluate(rb.velocity.magnitude) + oscillationSteerEffect, 0);
            fPhysicsWheelConfigJoint.axis = new Vector3(1, 0, 0);
            float TrainerCadence = GameObject.Find("FitnessEquipmentDisplay").GetComponent<FitnessEquipmentDisplay>().cadence;
            //Power Control. Wheel Torque + Acceleration curves
            float InstantaneousPower = GameObject.Find("FitnessEquipmentDisplay").GetComponent<FitnessEquipmentDisplay>().instantaneousPower;
            float TrainerSpeed = GameObject.Find("FitnessEquipmentDisplay").GetComponent<FitnessEquipmentDisplay>().speed;

            if (BlyncSpeed > 0 && !triggerPress.GetState(SteamVR_Input_Sources.RightHand))
                {
                    Vector3 targetVelocity = transform.forward * BlyncSpeed;
                    //rb.velocity = targetVelocity;
                    rWheelRb.AddTorque(transform.right * BlyncSpeed * 3.5f);
                }
            
            //rWheelRb.AddTorque(transform.right * calculatedTorque);
            ...
            //Handles
            cycleGeometry.handles.transform.localRotation = Quaternion.Euler(0, steeringValue * steerAngle.Evaluate(currentSpeed) + oscillationSteerEffect * 5, 0) * initialHandlesRotation;

            //LowerFork
            cycleGeometry.lowerFork.transform.localRotation = Quaternion.Euler(0, steeringValue * steerAngle.Evaluate(currentSpeed) + oscillationSteerEffect * 5, steeringValue * -axisAngle) * initialLowerForkLocalRotaion;
            ...
            cycleGeometry.fWheelVisual.transform.rotation = Quaternion.Euler(xQuat * (steeringValue * -axisAngle), steeringValue * steerAngle.Evaluate(currentSpeed) + oscillationSteerEffect * 5, zQuat * (steeringValue * -axisAngle));
            ...
            if (Physics.Raycast(transform.position + new Vector3(0, 1f, 0), Vector3.down, out hit, Mathf.Infinity))
            {
                ...
                if (hit.distance > airTimeSettings.heightThreshold && airTimeSettings.freestyle)
                {
                    ...
                    rb.AddTorque(Vector3.up * steeringValue * 4 * airTimeSettings.airTimeRotationSensitivity, ForceMode.Impulse);
        ...
        // private float CalculateSteeringValue()
        // {
        //     float currentZ = leftHandPose.GetLocalPosition(SteamVR_Input_Sources.LeftHand).z;
        //     float deadZone = 0.05f; // Tolerance for considering the position as straight

        //     // Determine if the current position is within the dead zone around the straight position
        //     if (Mathf.Abs(currentZ - straightPositionZ) <= deadZone)
        //     {
        //         // The hand is in the "straight" zone, so no steering should occur
        //         return 0f;
        //     }
        //     else if (currentZ < straightPositionZ)
        //     {
        //         // Adjust calculation for Left Turn, ensure currentZ is correctly between the expected range
        //         float leftSteer = Mathf.InverseLerp(straightPositionZ, leftTurnPositionZ, currentZ);
        //         return leftSteer * -1f; // Ensure negative value for left turn
        //     }
        //     else
        //     {
        //         // Right turn
        //         return Mathf.InverseLerp(straightPositionZ + deadZone, rightTurnPositionZ, currentZ);
        //     }
        // }
